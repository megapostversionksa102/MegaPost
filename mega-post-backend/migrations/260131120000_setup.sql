...
amazon_credential_id text,
 amazon_credential_secret text,
...
AND scheduled_at <= (NOW() AT TIME ZONE 'Asia/Riyadh')
...
BEGIN
            SELECT content::jsonb INTO sign_result FROM http_post(
                ...
                jsonb_build_object(
                    'asin', item.asin,
                    'amazonCredentials', jsonb_build_object(
                        'credentialId', set.amazon_credential_id,
                        'credentialSecret', set.amazon_credential_secret,
            ...
            new_price := (sign_result->'offersV2'->'listings'->0->'price'->'money'->>'amount')::float8;
            new_image := COALESCE(
                sign_result->'images'->'primary'->'large'->>'url',
                sign_result->'images'->'variants'->0->'large'->>'url'
            );
            ...
            IF new_price IS NOT NULL AND new_price > 0 THEN
                UPDATE products SET 
                    ...
                    price = new_price, 
                    last_update = (NOW() AT TIME ZONE 'Asia/Riyadh')
        ...
        SELECT TO_CHAR(NOW() AT TIME ZONE 'Asia/Riyadh', 'HH12:MI ') ||
        CASE WHEN TO_CHAR(NOW() AT TIME ZONE 'Asia/Riyadh', 'AM') = 'AM' THEN 'ص' ELSE 'م' END INTO time_part;

        final_price := ROUND((item.price * (1 - (COALESCE(item.extra_payment_discount, 0)::float / 100)))::numeric, 2);
        old_price_val := ROUND(COALESCE(item.old_price, 0)::numeric, 2);
...
price_display := '<b>' || TO_CHAR(final_price, 'FM999990.00') || ' ر.س</b>';
IF old_price_val > final_price THEN
price_display := price_display || ' (بدلاً من <s>' || TO_CHAR(old_price_val, 'FM999990.00') || ' ر.س</s>)';
END IF;

savings_line := '';
IF COALESCE(item.extra_payment_discount, 0) > 0 THEN
savings_line := '✨ وفر ' || item.extra_payment_discount || '% عند الدفع';
ELSIF COALESCE(item.discount, 0) > 0 THEN
savings_line := '🔥 خصم ' || item.discount || '%';
END IF;

IF item.footer_text IS NOT NULL AND trim(item.footer_text) <> '' THEN
v_processed_footer := replace(replace(item.footer_text, '((', ''), '))', '');
v_processed_footer := regexp_replace(v_processed_footer, '(https?://[^\s)]+)', '<a href="\1">اضغط هنا</a>', 'g');
END IF;
...
CASE COALESCE(item.template_id, 0)
        WHEN 1 THEN
            final_text := v_clean_title || ' بسعر <b>' || TO_CHAR(final_price, 'FM999990.00') || ' ر.س</b>';
            IF old_price_val > final_price THEN 
                final_text := final_text || ' (بدلاً من <s>' || TO_CHAR(old_price_val, 'FM999990.00') || ' ر.س</s>)'; 
            END IF;
            final_text := final_text || chr(10) || chr(10) || 'رابط العرض : ' || item.affiliate_link;

        WHEN 2 THEN
            final_text :=  v_clean_title || chr(10) ||
                          '┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄' || chr(10) ||
                          '💰 السعر النهائي: <b>' || TO_CHAR(final_price, 'FM999990.00') || ' ر.س</b>' || 
                          CASE WHEN old_price_val > final_price THEN ' (بدلاً من <s>' || TO_CHAR(old_price_val, 'FM999990.00') || ' ر.س</s>)' ELSE '' END || chr(10) ||
                          CASE 
                            WHEN COALESCE(item.discount, 0) > 0 OR COALESCE(item.extra_payment_discount, 0) > 0 THEN
                                '📉 خصم ' || COALESCE(item.discount, 0) || '%' ||
                                CASE WHEN COALESCE(item.extra_payment_discount, 0) > 0 THEN ' + ✨ وفر ' || item.extra_payment_discount || '% عند الدفع' ELSE '' END || chr(10)
                            ELSE '' 
                          END ||
                          '┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄' || chr(10) ||
                          '🔗 رابط العرض:' || chr(10) || 
        ...
        ELSE
            final_text := v_clean_title || chr(10) || chr(10) || 
                '<blockquote>🟢 السعر النهائي ⇚ <b>' || TO_CHAR(final_price, 'FM999990.00') || ' ر.س</b>' || 
                CASE WHEN old_price_val > final_price THEN ' (بدلاً من <s>' || TO_CHAR(old_price_val, 'FM999990.00') || ' ر.س</s>)' ELSE '' END ||
                CASE 
                    WHEN COALESCE(item.discount, 0) > 0 OR COALESCE(item.extra_payment_discount, 0) > 0 THEN
                        chr(10) || 
                        CASE WHEN COALESCE(item.discount, 0) > 0 THEN '🔥 خصم ' || item.discount || '%' ELSE '' END ||
                        CASE WHEN COALESCE(item.discount, 0) > 0 AND COALESCE(item.extra_payment_discount, 0) > 0 THEN ' ، ' ELSE '' END ||
                        CASE WHEN COALESCE(item.extra_payment_discount, 0) > 0 THEN '✨ وفر ' || item.extra_payment_discount || '% عند الدفع' ELSE '' END
                    ELSE '' 
                END || '</blockquote>';
                
            final_text := final_text || chr(10) || chr(10) || '🔗 رابط المنتج:' || chr(10) || item.affiliate_link;
    END CASE;
END IF;
...
fb_clean_text := regexp_replace(fb_clean_text, '<[^>]*>', '', 'g');
        FOR current_chat IN SELECT unnest(ARRAY[set.tg_chat_id, set.tg_chat_id_2, set.tg_chat_id_3])
        LOOP
            IF current_chat IS NOT NULL AND current_chat <> '' THEN
                BEGIN
                    SELECT content::jsonb INTO tg_result FROM http_post(
                        ...
                        jsonb_build_object(
                            ...
                            'caption', final_text, 
                            'parse_mode', 'HTML',
                            'disable_notification', COALESCE(item.disable_notification, false)
                        )::text,
                        'application/json');
                    IF tg_result->>'ok' = 'true' AND v_tg_msg_id IS NULL THEN 
                        v_tg_msg_id := (tg_result->'result'->>'message_id')::int8; 
                    END IF;
        ...
        IF set.fb_page_id IS NOT NULL AND set.fb_page_id <> '' AND real_fb_token IS NOT NULL THEN
            BEGIN
                SELECT content::jsonb INTO fb_result FROM http_post(
                    ...
                    'application/json');
                IF fb_result->>'id' IS NOT NULL THEN v_fb_post_id := set.fb_page_id || '_' || (fb_result->>'id'); END IF;
            EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'Facebook Failed'; END;
        END IF;

        IF v_tg_msg_id IS NOT NULL OR v_fb_post_id IS NOT NULL THEN
            UPDATE products SET 
                published_at = (NOW() AT TIME ZONE 'Asia/Riyadh')::text,
                scheduled_at = NULL,
